...
public:
	...
	void * proc_addr(const char* sym)
		...
		if ( hdl_ == NULL || sym == NULL )
			{
			fprintf(stderr, "Invalid handle, call load first.");
			return addr;
			}
...
public:
	...
	proc_t & operator () (void)
        ...
        if (0 == proc_)
        {
            fprintf(stderr, "NULL function %s pointer", sp::tochar(name_.c_str()).c_str());
        }
		return proc_;
		}
...
public:
	...
	virtual int open(const char_t * lib)
		...
		if ( !lib_loader_.load(sp::tochar(lib_name_.c_str()).c_str()) )
			{
			fprintf(stderr, "su_lib::open | cannot load lib %s, errno %d.", sp::tochar(lib_name_.c_str()).c_str(), error_code());
			return -1;
			}
	...
	virtual int open(handle hdl, bool no_free)
		...
		if ( !lib_loader_.load(hdl, no_free) )
			{
			fprintf(stderr, "su_lib::open | cannot load lib 0x%x.", hdl);
			return -1;
			}
		return load_procs();
		}
...
#define su_proc_load_return(cmd_name)					\
	su_proc_load_action(cmd_name,	 LOG_ERROR("load proc %s failed.", #cmd_name)
#define su_proc_load(cmd_name)							\
	su_proc_load_action(cmd_name,	 LOG_WARN( "load proc %s failed.", #cmd_name))
#define su_proc_load_w(cmd_name)						\
	su_proc_load_action_w(cmd_name,	 LOG_WARN(L"load proc %s failed.", L## #cmd_name))
