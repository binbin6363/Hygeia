...
namespace sp
{
	...
	*/
	...
	int search_strarr(const char_t * str, const char_t (&arr)[row][col], bool case_sensitive = true);
	...
	template <typename char_t, int count> inline
	...
	const char * itostr(int val, std::string & str, const char * fmt = 0);
	const wchar_t * itostr(int val, std::wstring & str, const wchar_t * fmt = 0);
	std::string itostr(int val, const char * fmt = 0);
	std::wstring itowstr(int val, const wchar_t * fmt = 0);
	const char * ftostr(double val, std::string & str, const char * fmt = 0);
	const wchar_t * ftostr(double val, std::wstring & str, const wchar_t * fmt = 0);
	...
	/** 
	 * @fn		strcopy
	 * @brief	字符串拷贝，增强的可以进行宽窄转换的
	 * @author	jdyu
	 * @date	2008/10/13
	 * @return	const char * 返回目标缓冲区 
	 */
...
namespace sp
{
	 ...
	 * @see		
	 */
	int cat_audio_file(const char * file, const void * data, size_t bytes, bool fail_if_exist = false);
...
namespace sp
{
	 ...
	 * @exception 
	 */
	char* get_module_cfg(char path[MAX_PATH], const char* cfg_default = 0, IU_HANDLE hmod = 0);
	std::string get_module_cfg(const char * cfg_default = 0, IU_HANDLE hmod = 0);
