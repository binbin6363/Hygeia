...
public:
	...
	int free(ATBUF * ab)
		{
		GUARD gd(bufs_lock_);
		bufs_map::iterator pi = bufs_map_.find(ab);
		if ( pi == bufs_map_.end() )
			{
	...
	// 支持单体访问
	static auto_buf_mngr<ATBUF, MUTEX> & instance(void)
		{
		static auto_buf_mngr<ATBUF, MUTEX> the_ab_mngr;
		return the_ab_mngr;
		}
